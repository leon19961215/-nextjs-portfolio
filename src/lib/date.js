...
export function getDashedDate (date) {
  ...
  const day = padWithZeros(dateObj.getDate(), 2)
  const month = padWithZeros(dateObj.getMonth() + 1, 2)
  const year = padWithZeros(dateObj.getFullYear(), 2)
  return `${year}-${month}-${day}`
}

export function isNowExpired (expireAt) {
  const expireAtObj = new Date(expireAt)
  return Date.now() - expireAtObj.getTime()>0
}